...
# ----------------- Environment tags -----------------
def _git_cmd(args: List[str], cwd: Optional[str] = None) -> Optional[str]:
    try:
...
def gather_env_tags(cwd: Optional[str] = None) -> List[str]:
    """Collect git repo, branch, username, and hostname as tags (best-effort)."""
    tags: List[str] = ["claude-code"]

    # Git repo name (basename of the toplevel directory)
    toplevel = _git_cmd(["rev-parse", "--show-toplevel"], cwd=cwd)
    if toplevel:
    ...
    # Username
    try:
        tags.append(f"user:{getpass.getuser()}")
    except Exception:
        pass

    # Hostname
    try:
        tags.append(f"host:{socket.gethostname()}")
    except Exception:
        pass

    # OS platform
    tags.append(f"os:{sys.platform}")
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            tags.append(f"cc-version:{result.stdout.strip()}")
    except Exception:
        pass
...
def emit_turn(langfuse: Langfuse, session_id: str, turn_num: int, turn: Turn, transcript_path: Path, tags: Optional[List[str]] = None, host: str = "", public_key: str = "", secret_key: str = "") -> None:
    ...
    with langfuse.start_as_current_observation(
        ...
        # Set tags via direct API call
        try:
            trace_id = langfuse.get_current_trace_id()
            info(f"Trace ID: {trace_id}, tags: {tags}")
            if trace_id and tags:
                import httpx
                resp = httpx.post(
                    ...
                    json={
                        "batch": [{
                            ...
                            "body": {"id": trace_id, "tags": tags},
                        }],
                        "metadata": {},
                    },
                    timeout=10,
                )
                info(f"Tags API response: {resp.status_code} {resp.text[:200]}")
            else:
                warn(f"Skipping tags: trace_id={trace_id}, tags={tags}")
        except Exception as e:
            import traceback
            error(f"Failed to set tags: {e}\n{traceback.format_exc()}")
    ...
    try:
        with FileLock(LOCK_FILE):
            ...
            for t in turns:
                ...
                try:
                    debug(f"Emitting turn {turn_num} for session {session_id}")
                    emit_turn(langfuse, session_id, turn_num, t, transcript_path, tags=tags, host=host, public_key=public_key, secret_key=secret_key)
                    debug(f"Turn {turn_num} emitted successfully")
                except Exception as e:
                    import traceback
        ...
        try:
            debug("Flushing langfuse...")
            langfuse.flush()
            debug("Flush complete")
        except Exception as e:
            error(f"Flush failed: {e}")
